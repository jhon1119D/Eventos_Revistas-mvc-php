    ...
    <table class="borde">
        ...
        <tbody class="informacion-datos"> <?php foreach ($datos as $eventos): ?>
                <tr>
                    ...
                    <td><a href="<?php echo s($eventos->enlace); ?>" target="_blank"><?php echo s($eventos->enlace); ?></a></td>
                    <td><?php echo s($eventos->ranking); ?></td>
                    <td><?php echo s($eventos->acronimo); ?></td>
                    <td><?php echo s($eventos->fecha); ?></td>
                    <td>
                        <?php if (!empty(trim($eventos->documento_url)) && trim($eventos->documento_url) != 'documentos/eventos/'): ?>
                            <!-- Enlace de descarga -->
                            <a href="documentos/eventos/<?php echo $eventos->documento_url; ?>" download>
